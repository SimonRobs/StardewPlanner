...
class LibraryObjectBuilder {
    ...
    static func buildObject(from definition: LibraryObjectDef) -> ScenePlaceable? {
        ...
        var object: ScenePlaceable = LibraryObject(ofType: definition.type, withSize: objectSize)
        
        if let area = definition.metadata?.area {
            object = RangedDecorator(object, area: area)
        }
        
        if definition.metadata?.hasBuildingShadow != nil {
            object = ShadowedDecorator(object)
        }
